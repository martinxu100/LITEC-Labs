...
// Global variables
char input;							//store keyboard entry, Error 1. added semicolon to end
char number;
...
void main(void) 	       			// Start main function
{
  	...
  	while(10)      					// Begin infinite loop 
  	{
		...
		else if (input == 'F')		// ECSE department chosen, Error 2. changed = to ==
		{
			Fall();
		}
		else if (input == 'W')
...
void Spring(void)             // Error 4. fixed the spelling for the "void" in the function Spring
{
	printf("No surprise. \r\n");
